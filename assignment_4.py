...
def exercise_01():
    ...
    df = df.dropna()
    df.drop(df.index[df['year'] == 'XXXX'], inplace = True)
    df.drop(df.index[df['name'] == '_PRENOMS_RARES'], inplace = True)
    ...
    df = df.astype({'year':'int64', 'name':object, 'gender':object, 'births':'int64'}) 
